...
No Maven, no npm, no clone — two files and Docker. **Dashboard** → http://localhost:5173 &nbsp;|&nbsp; **API Docs** → http://localhost:5173/docs
...
CI publishes both as workflow artifacts (`jacoco-aggregate-report`, `vitest-coverage-report`) on every run — see `.github/workflows/ci.yml`. There is deliberately no coverage badge: the per-module JaCoCo and Vitest thresholds in the poms and `vite.config.ts` are the gate that actually fails a build, and a hand-edited shields.io number reads like a measurement without being one.
...
| What | How | Default |
|------|-----|---------|
| **API reference** | http://localhost:5173/docs | Always on, no login |
| **Swagger UI** | http://localhost:8080/swagger-ui.html | Off — set `SWAGGER_ENABLED=true` |
