...
create-topics: ## Create Kafka topics (idempotent)
	...
	@docker exec webhook-kafka /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --create --if-not-exists --topic deliveries.dispatch --partitions 3 --replication-factor 1 2>/dev/null || true
	@docker exec webhook-kafka /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --create --if-not-exists --topic deliveries.retry.1m --partitions 3 --replication-factor 1 2>/dev/null || true
	@docker exec webhook-kafka /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --create --if-not-exists --topic deliveries.retry.5m --partitions 3 --replication-factor 1 2>/dev/null || true
	@docker exec webhook-kafka /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --create --if-not-exists --topic deliveries.retry.15m --partitions 3 --replication-factor 1 2>/dev/null || true
	@docker exec webhook-kafka /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --create --if-not-exists --topic deliveries.retry.1h --partitions 3 --replication-factor 1 2>/dev/null || true
	@docker exec webhook-kafka /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --create --if-not-exists --topic deliveries.retry.6h --partitions 3 --replication-factor 1 2>/dev/null || true
	@docker exec webhook-kafka /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --create --if-not-exists --topic deliveries.retry.24h --partitions 3 --replication-factor 1 2>/dev/null || true
	@docker exec webhook-kafka /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --create --if-not-exists --topic deliveries.dlq --partitions 3 --replication-factor 1 2>/dev/null || true
...
restore-db: ## Restore embedded database from backup (usage: make restore-db FILE=backups/webhook_platform_20241217_120000.sql.gz)
	...
	@echo "$(YELLOW)⚠️  WARNING: This will DROP and recreate the database$(NC)"
...
nuke: ## DESTROY EVERYTHING including volumes (requires CONFIRM=YES)
	...
	@docker stop webhook-api webhook-worker webhook-ui webhook-postgres webhook-kafka webhook-minio 2>/dev/null || true
	@docker rm -f webhook-api webhook-worker webhook-ui webhook-postgres webhook-kafka webhook-minio 2>/dev/null || true
	@docker volume rm webhook_pgdata kafka_data minio_data 2>/dev/null || true
