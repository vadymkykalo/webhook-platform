...
public interface IncomingForwardAttemptRepository extends JpaRepository<IncomingForwardAttempt, UUID> {
        ...
        /**
         * Claim for first dispatch or replay. The token is generated by the caller rather
         * than in SQL so the attempt that won the claim knows the value it must still match
         * when it finalises (V060) — the same reason deliveries.claim_token is caller-supplied.
         */
        @Modifying
        ...
        @Modifying
        // claim_token is cleared alongside the status: taking a claim away has to end it, or
        // the attempt that was swept could still match its own token and finalise a row it no
        // longer owns. "Token set" means "currently claimed", exactly as for deliveries (V055).
