...
export default function AddMemberModal({ orgId, open, onClose, onSuccess }: AddMemberModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // The invite token is emailed, never returned to the browser.
      showSuccess(t(response.status === 'INVITED' ? 'members.toast.invited' : 'members.toast.added'));
      handleClose();
      onSuccess();
    } catch (err: any) {
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>{t('members.addModal.description')}</DialogDescription>
        </DialogHeader>
